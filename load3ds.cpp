...
bool Load3DS(char *filename, ID3D11Device* g_pd3dDevice, ID3D11Buffer **ppVertexBuffer, int *vertex_count, BOUNDARY_TYPE b, Model* model, bool gouraud)
{
	...
	int vv = 0;
	XMFLOAT3 avg(0, 0, 0);
	XMFLOAT3 max_x, min_x, max_y, min_y, max_z, min_z;
	...
	for (int uu = 0; uu < submodels.size(); uu++)
		for (int ii = 0; ii < submodels[uu].ianz; ii++)//weil 1.dreieck nix data.... anz--;oder eben ii=1 und net 0
		{
			...
			if (vv == 0)
				max_x = min_x = max_y = min_y = max_z = min_z = noIndexVer[vv].Pos;


			if (b == SPHERE)
			{
				//calculate average vertex
				avg.x += noIndexVer[vv].Pos.x;
				avg.y += noIndexVer[vv].Pos.y;
				avg.z += noIndexVer[vv].Pos.z;
			}
			if (b == BOX)
			{
				if (noIndexVer[vv].Pos.x > max_x.x)
				{
					max_x = noIndexVer[vv].Pos;
				}
				else if (noIndexVer[vv].Pos.x < min_x.x)
				{
					min_x = noIndexVer[vv].Pos;
				}

				if (noIndexVer[vv].Pos.y > max_y.y)
				{
					max_y = noIndexVer[vv].Pos;
				}
				else if (noIndexVer[vv].Pos.y < min_y.y)
				{
					min_y = noIndexVer[vv].Pos;
				}

				if (noIndexVer[vv].Pos.z > max_z.z)
				{
					max_z = noIndexVer[vv].Pos;
				}
				else if (noIndexVer[vv].Pos.z < min_z.z)
				{
	...
	*vertex_count = vertex_anz;
	...
	if (b == BOX)
	{
		model->boundary = new Box_Boundary(max_x, max_y, max_z, min_x, min_y, min_z);
	}
	if (b == SPHERE)
	{
		avg.x /= vertex_anz;
		...
		//determine the vertex furthest away from average
		float radius = 0.0f;
		XMFLOAT3 radiusVec;
		for (int i = 0; i < vertex_anz; i++)
		{
			XMFLOAT3 newDistanceVec = noIndexVer[i].Pos - avg;
			float newDistance = sqrtf(newDistanceVec.x*newDistanceVec.x + newDistanceVec.y*newDistanceVec.y + newDistanceVec.z*newDistanceVec.z);
			if (newDistance > radius)
			{
				radius = newDistance;
				radiusVec = newDistanceVec;
			}
		}

		model->boundary = new Sphere_Boundary(avg, radius);

		/*ObjectSphereVec objSph;
		objSph.center = XMLoadFloat3(&avg);
...
bool LoadOBJ(char * filename, ID3D11Device * g_pd3dDevice, ID3D11Buffer ** ppVertexBuffer, int * vertex_count, bool gourad)
{
	ID3D11Buffer *pVertexBuffer = NULL;
